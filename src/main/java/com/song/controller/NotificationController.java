...
import com.song.service.NotificationService;
...
public class NotificationController {
    ...
    public Result domerCreate(@RequestBody Notification notification, HttpServletRequest request){
        ...
        if (pageInfo.getList() != null && pageInfo.getList().size()>0){
            ...
            Building building = buildingService.detail(device.getBuildingId());
    ...
    public Map<String, Object> queryMyBuilding(HttpServletRequest request){
        ...
        Device device = deviceService.detail(user1.getDeviceId());
        System.out.println(device.getBuildingId());
        int buildingId = device.getBuildingId();
        System.out.println("编号："+buildingId);

        PageInfo<Notification> pageInfo = notificationService.queryMyBuilding(buildingId);
    ...
    public Map<String, Object> queryMyDevice(HttpServletRequest request){
        User param = (User)request.getAttribute("user");
        int userId = param.getId();     // 1. 获取该用户Id

        PageInfo<Notification> pageInfo = notificationService.queryMyDevice(userId);  // 通过用户ID查询通知
